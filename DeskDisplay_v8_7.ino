...
SystemState sys;
DinoGame dinoGame(display);

// Button handler (v8.7: FSM unica, vedi button_handler.h/.cpp)
...
String clockStyleName(ClockAnimStyle style) {
  switch (style) {
    ...
    case CLOCK_STYLE_WAVE: return "STYLE: WAVE";
    case CLOCK_STYLE_ORBIT: return "STYLE: ORBIT";
    case CLOCK_STYLE_BINARY: return "STYLE: BINARY";
    case CLOCK_STYLE_STARFIELD: return "STYLE: STARFIELD";
...
void goToScreen(ScreenMode mode) {
  ...
  if (mode == SCREEN_GAME) dinoGame.reset();
}
...
void handleSingleClick() {
  if (sys.currentScreen == SCREEN_GAME) return;  // salto già gestito su BTN_PRESS_START
  Serial.println("[BUTTON] CLICK -> schermata successiva");
  cycleScreen();
}

void handleDoubleClick() {
  if (sys.currentScreen == SCREEN_GAME) return;  // nessun significato in gioco
...
void buttonCallback(const ButtonEvent& event) {
  switch (event.type) {
    case BTN_PRESS_START:
      ...
      buzzer.playButtonPress();
      // Nel gioco il salto deve scattare qui, non su BTN_CLICK: quest'ultimo
      // ha ~300ms di ritardo voluto (attesa doppio click) che renderebbe
      // il gioco ingiocabile.
      if (sys.currentScreen == SCREEN_GAME) dinoGame.jump();
      break;
...
void setup() {
  ...
  renderer.drawBootScreen("DESK", "DISPLAY", "v8.7");
  buzzer.begin();
...
void loop() {
  ...
  if (sys.currentScreen == SCREEN_GAME) {
    // v8.7: framerate più alto per il gioco, indipendente dal resto
    dinoGame.update();
    if (millis() - lastRenderRefresh > GAME_FRAME_MS) {
      lastRenderRefresh = millis();
      display.clearDisplay();
      dinoGame.render();
      display.display();
    }
  } else if (millis() - lastRenderRefresh > 250) {
    lastRenderRefresh = millis();
    renderer.render(sys);
  }
}
